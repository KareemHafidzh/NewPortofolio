...
const SkillItem = ({ name, logo }: Skill) => (
  <div className="group flex flex-col items-center justify-center text-center p-2 md:p-4 rounded-2xl transition-all duration-200 hover:bg-white/90 hover:backdrop-blur-sm hover:shadow-md">
    <img
      src={logo}
      alt={`${name} logo`}
      className="w-10 h-10 md:w-16 md:h-16 object-contain mb-1.5 md:mb-3 transition-transform duration-200 group-hover:scale-110"
    />
    <p className="text-[11px] md:text-sm font-semibold text-[#0135AD] leading-normal">
      {name}
    </p>
  </div>
);
